...
params['min_children'] = 5  # min # branches per parent object
params['max_children'] = 6  # max # branches per parent object
params['dx_size'] = .55  # scale factor affecting how far away children are from their parent object
params['stretch_size'] = 0.2  # factor affecting how much spheres are stretched
...
params['pruning_max_scale'] = 1  # Objects with larger scale are not plotted. =max obj scale in gen1 = scale_factor.
params['base_color'] = np.array([0.5, 0.5, 0.5])  # get_random_base_color()  # e.g. np.array([0.5, 0.5, 0.5])
params['stretch'] = True  # If true, allows ellipsoid. If false, only spheres.
params['theta_max'] = np.pi / 2  # children are plotted on the outward facing surface of the parent. theta is the angle between vectors (c - p) and (p - g) where c, p, g are the position vectors of the Child, its Parent, and its Grandparent. max theta_max is pi.
